...
After I reviewed my UML class diagram, I noticed a handful of design choices I made that I could improve upon if I had more time to work on the project. Most notably, RestDay implements WorkoutPlan but violates the Liskov Substitution Principle by which it returns empty ArrayLists for methods like getExercises() and zero for methods like getDuration(). This heirarchy forced me to write UI code that checks if a WorkoutPlan is a Workout or RestDay, which defeated the purpose of polymorphism. While the UML shows the inheritance structure, it fails to capture this behavioral incompatibility. I would refactor this issue by creating separate interfaces: an ExerciseContainer for Workout, and a ScheduleItem interface for both classes and refactor the UI to work with these new explicit types. Furthermore, my ExerciseAssociator class currently implements an Observer pattern informally, where Equipment and Muscle track metrics from associated exercises through manual registerExercise() method calls (The process is as follows: When a Workout is added to WeeklySchedule, it calls an activateMetrics function on all of its Exercise objects. Each Exercise object calls the ExerciseAssociator activate method on its Equipment (ExerciseAssociator instance). Lastly, MuscleGroup calls the activateMetrics() method on each of its Muscle objects). Similarly, this important relationship is completely absent from the UML diagram. To improve upon this design choice, I would make WeeklySchedule a Subject with notify methods and have Equipment and Muscle (or perhaps MuscleGroup---see further below in the next paragraph) implement an Observer interface. This change would allow them to automatically update when metrics change without any manual calls at the UI-level, which also would make this pattern explicit in both code and my UML diagram. Finally, the dependency between WorkoutLibrary and WeeklySchedule, and Exercise to ExerciseLibrary and Workout involves another invisible relationship in the UML diagram. When I remove a Workout from WorkoutLibrary in my UI, I manually check if it exists in WeeklySchedule. Similarly, when I remove an Exercise from ExerciseLibrary, I manually check if it exists in any Workout in WorkoutLibrary. I fully intended for this relationship to occur naturally---if I were to apply the Observer pattern here, it would trigger automatic updates between these components without manual dependency checks and would clarify this intended relationship in the UML diagram design.

The relationship between MuscleGroup and Muscle also presents problems that the UML fails to illustrate. For instance, "Quadriceps" appears both as a Muscle and MuscleGroup, and muscles like "Lower Back" appear in multiple MuscleGroups. My current implementation uses a simple container model where MuscleGroup acts like a folder that can only contain Muscle objects, but not other MuscleGroups. This doesn't accurately represent the natural hierarchical structure of the muscular system, where a muscle group can contain both individual muscles and smaller muscle groups (like how the "Legs" (depending on the exercise) might contain the groin muscle and the "Quadriceps" group). This detail results in huge differences in aggregated metrics across muscle groups and muscles due to the increased accuracy and reduced redundancy possible with this new relationship. If I implemented the Composite pattern, both Muscle and MuscleGroup would implement a common Component interface with shared operations. This enhancement would allow me to more simply manage aggregating metrics automatically (with the ExerciseAssociator improvement explained) at arbitrary depths of Muscle-MuscleGroup complexity, while still maintaining consistent behavior regardless of whether an operation is performed on a single muscle or an entire group of muscles. Furthermore, while I believe I have done a relatively good job on minimizing code duplication through interfaces and abstractions, the UI components ExerciseManagementPanel and WorkoutManagementPanel have nearly identical methods like createExerciseList/createWorkoutList and createDetailsPanel. Both also follow the same layout with a list on the left, details on the right, and action buttons at the bottom. However, the UML class diagram does not show or indicate this behavioral duplication coupling. So, if I had time to create an abstract EntityManagementPanel superclass, I would extract this common structure into an abstract class (like WorkoutEditPanel). Such a change will positively benefit with creating management panels for more complexity introduced in the model package, and will enhance the detail in the UML diagram-depicted class hierarchy regarding how the program functions as a whole.
